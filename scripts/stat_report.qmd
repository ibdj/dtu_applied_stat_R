...
df <- as.data.frame(summary(pca_stat_veg_ggplot))

ggplot(scores, aes(x = `PC 1`, y = `PC 2`)) +
  geom_point(size = 2, alpha = 0.6, color = "blue") +
  #theme_minimal() +
  labs(title = "PCA Score Plot",
       x = paste0("PC1 (", round(pca_stat_veg_ggplot[1,1], 1), "%)"),
       y = paste0("PC2 (", round(pca_stat_veg_ggplot[1,2], 1), "%)"))
